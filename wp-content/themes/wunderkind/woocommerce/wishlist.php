...
<form id="yith-wcwl-form" action="<?php echo esc_url( $yith_wcwl->get_wishlist_url() ) ?>" method="post">
    ...
    { echo apply_filters( 'yith_wcwl_wishlist_title', '<h3><i class="fa fa-shopping-cart"></i> ' . $wishlist_title . '</h3>' ); }
    do_action( 'yith_wcwl_before_wishlist' );
    ?>
    <table class="cart-table wishlist_table" cellspacing="0">
        <thead>
        <tr>
            <th class="product-name" colspan="2"><span class="nobr"><?php _e( 'Product Name', 'yit' ) ?></span></th>
            ...
            <?php if( get_option( 'yith_wcwl_add_to_cart_show' ) == 'yes' ) : ?><th colspan="2"><span class="nobr"></th><?php endif ?>
        </tr>
        </thead>
        <tbody>
        <?php
            ...
            foreach( $wishlist as $values ) :
                ...
                if( $product_obj !== false && $product_obj->exists() ) : ?>
                    <tr id="yith-wcwl-row-<?php echo $values['ID'] ?>">
                        ...
                        <?php endif ?>
                        <td class="product-remove"><div><a href="javascript:void(0)" onclick="<?php echo $remove_wishlist ?>" class="remove" title="<?php _e( 'Remove this product', 'yit' ) ?>"><i class="fa fa-times"></i></a></td>
                    </tr>
                <?php
                endif;
            endforeach;
